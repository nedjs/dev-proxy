...
program.version(pack.version)
	...
	.option('--handler <path>', 'Javascript proxy handler file, use template: \n\
		module.exports = { \n\
			onProxyRequest: (proxyReq, request, response) => {}, \n\
			onProxyResponse: (proxyRes, request, response) => {} \n\
...
if(program.handler) {
	addCustomRequestHandler(proxyServer, app);
}
if(program.cors) {
...
function addCustomRequestHandler(proxyServer, app) {
	let handlerFile = path.isAbsolute(program.handler) ? program.handler : path.join(fs.realpathSync(process.cwd()), program.handler);
	console.log('Loading handler: ' + program.handler);
	...
	if(requestHandler.onProxyRequest && typeof requestHandler.onProxyRequest !== "function") {
		console.error('Request handler function "onProxyRequest" is not a function.');
		process.exit(1);
	}
	if(requestHandler.onProxyResponse && typeof requestHandler.onProxyResponse !== "function") {
		console.error('Request handler function "onProxyResponse" is not a function.');
		process.exit(1);
	}
	if(requestHandler.middleware && typeof requestHandler.middleware !== "function") {
		console.error('Request handler function "middleware" is not a function.');
		process.exit(1);
	}

	proxyServer.on('proxyReq', function(proxyReq, req, res, options) {
		proxyReq.setHeader('X-Proxy', 'dev-proxy');
		if(requestHandler.onProxyRequest) {
			requestHandler.onProxyRequest(proxyReq, req, res);
		}
	});
	proxyServer.on('proxyRes', function(proxyRes, req, res, options) {
		res.setHeader('X-Proxy', 'dev-proxy');
		if(requestHandler.onProxyResponse) {
			requestHandler.onProxyResponse(proxyRes, req, res);
		}
	});

	if(requestHandler.middleware) {
		app.use(requestHandler.middleware);
	}
}
