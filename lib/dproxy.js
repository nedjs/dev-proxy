...
program.version(pack.version)
	...
	.option('-m, --map <remote>,<local>', 'Maps a remote request to a local path. May be specified multiple times. '+
										  'If the local path is relative then it will be resolved to the <dir> param. '+
	...
	.option('-l, --logLevel [level]', 'Logging levels are as follows: 0=off 1=all 2=messages 4=REGEX 8=MAP 16=LOCAL 32=REMOTE. '+
									  'Levels are bit masked so you can use any combination. EG: 14=messages,REGEX,MAP', 1)
...
app.use(function(req, res, next) {
	if(program.map && program.map.length > 0) {
		for(var i=0;i<program.map.length;i++) {
			...
			if(entry[0] === req.path || entry[0] === req.path.substring(1)) {
				if(LOG_MAP) {
